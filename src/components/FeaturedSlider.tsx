        ...
        <div className="relative overflow-visible">
          ...
          >
            {featured.map((product) => (
              ...
              >
                <div className="group relative bg-luxury-bg-secondary dark:bg-luxury-dark-card rounded-sm p-10 border border-luxury-bg-card dark:border-maroon-border/20 hover:border-gold/40 transition-all duration-700 shadow-[0_30px_70px_rgba(0,0,0,0.1)] dark:shadow-[0_30px_70px_rgba(0,0,0,0.5)] overflow-hidden mobile-card-shadow">
                  <div className="absolute top-8 right-8 z-20 w-14 h-14 border border-gold/10 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-700">
                    <Sparkles className="w-6 h-6 text-gold animate-pulse gold-glow" />
                  </div>

                  <div className="aspect-square mb-10 overflow-hidden relative bg-luxury-bg-card dark:bg-luxury-dark-secondary rounded-sm">
                    <img 
                      src={product.image} 
                      alt={product.name} 
                    ...
                    {product.isNewArrival && (
                      <div className="absolute top-0 left-0 bg-gold text-maroon-dominant text-[9px] px-5 py-2.5 uppercase tracking-[0.4em] font-black shadow-xl">
                        New Launch
                      </div>
                    )}
                  </div>
                  ...
                  <div className="text-center relative space-y-3">
                    <span className="text-gold text-[9px] font-black uppercase tracking-[0.5em] block opacity-80">Artisan's Choice</span>
                    <h4 className="font-serif text-maroon-dominant dark:text-white text-2xl mb-4 line-clamp-1 group-hover:text-gold transition-colors duration-500 uppercase tracking-wide">
                      {product.name}
                    </h4>
                    ...
                    <div className="flex items-center justify-center gap-2 mb-8">
                      ...
                      <span className="text-[10px] text-luxury-text-light/40 dark:text-luxury-text-darkMuted uppercase tracking-[0.3em] font-black">(BIS Hallmark)</span>
                    </div>

                    <div className="relative mb-10">
                       <p className="text-gold font-serif text-3xl font-bold tracking-tight gold-glow">
                        ₹{product.price.toLocaleString('en-IN')}
                      </p>
                      <div className="w-16 h-[1px] bg-gold/20 mx-auto mt-4"></div>
                    </div>

                    <button className="relative w-full py-5 border border-gold/50 text-gold text-[10px] font-black uppercase tracking-[0.4em] overflow-hidden group/btn transition-all duration-500 hover:text-white dark:hover:text-maroon-dominant">
                      <span className="relative z-10 flex items-center justify-center gap-3">
                        EXPLORE THE CRAFT <ArrowRight className="w-3 h-3 group-hover/btn:translate-x-3 transition-transform" />
                      </span>
                      <div className="absolute inset-0 bg-gold transform -translate-x-full group-hover/btn:translate-x-0 transition-transform duration-700 ease-out"></div>
                    </button>
                  </div>
                </div>
              </div>
