...
const ProductCard: React.FC<ProductCardProps> = ({ product, onClick, onToggleWishlist, isWishlisted, onAddToCart }) => {
  return (
    <div className="group relative bg-white dark:bg-luxury-dark-card border border-transparent dark:border-white/5 hover:border-gold/20 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 cursor-pointer rounded-[2rem] h-full flex flex-col p-3 md:p-4">
      {/* Image */}
      <div className="relative aspect-square overflow-hidden bg-luxury-bg-card dark:bg-luxury-dark-secondary rounded-2xl mb-3">
        <img
          src={product.image}
          alt={product.name}
          loading="lazy"
        ...
        {product.isNewArrival && (
          <div className="absolute top-3 left-3 bg-gold/90 backdrop-blur-sm text-maroon-dominant text-[8px] px-3 py-1.5 uppercase tracking-[0.3em] font-black shadow-lg rounded-full">
            New Launch
          </div>
        )}
        <button
